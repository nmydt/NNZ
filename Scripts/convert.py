...
def formatUrl(url):
    if url[-1]=="/":
        ...
        return url
#查找后缀文件
def findAllSuffix(target_dir, target_suffixs=["md","html"]):
    find_res = []
    for target_suffix in target_suffixs:
...
def SingleConvert(filePath,repo,index):
    ...
    if index==1:  #     github => cdn       
        ...
        ## html
        match = list(filter(lambda x:"https://cdn.jsdelivr.net/" not in x, re.findall(r'src="(.*?)"',r)))
        tuple_ = list(zip(match,list(map(lambda y:UrlFactory(y),map(lambda x:f"{repo}/"+x,match)))))
        for i in tuple_:
            r = r.replace(i[0],i[1])
